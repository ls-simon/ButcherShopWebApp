...
mongoConn.once('open', function() {
    ...
    for(var i = 0; i<data.length; i++){
        var produkt = capitalizeFirstLetter(data[i]['Produkt'].toLowerCase());
        var pris = data[i]['Pris'];
        var enhed = data[i]['Enhed'] == undefined ? 'stk' : data[i]['Enhed'].toLowerCase();

        if(data[i]['Pris'] == "kategori"){
            currentKategori = produkt;

                if (!contains(kategorier, currentKategori)) {
                    kategorier.push(currentKategori);

                    var vareKategori = new VareKategori({
                        navn: currentKategori
                    });
